...
// @todo: move this component to separate file
//
Vue.component('full-topology', {
  ...
  methods: {
    buildTopology: function() {
        websocketHandler.addConnectHandler(() => {
...
Vue.component('app-topology', {
  ...
  computed: {
    possibleToBuildHierarchiedTopology: function() {
      return this.layout.dataManager.nodeManager.nodes.isThereAnyNodeWithType("libvirt");
    }
  }
});
...
var TopologyComponentNewApproach = {
    ...
    <div class="topology">\
      ...
      <div class="col-sm-7 fill content">\
        ...
        <div class="topology-controls">\
          ...
          <button v-if="layoutType === \'host\'" v-if="currentNode != null && currentNode.isGroupOwner()" id="expand-all" type="button" class="btn btn-primary" \
  ...
  mounted: function() {
    ...
    this.$store.subscribe(function(mutation) {
      ...
      if (mutation.type === "highlight")
        skydiveInfraLayout.reactToTheUiEvent('node.highlight.byid', mutation.payload);
      else if (mutation.type === "unhighlight")
        skydiveInfraLayout.reactToTheUiEvent('node.unhighlight.byid', mutation.payload);
      else if (mutation.type === "emphasize")
        skydiveInfraLayout.reactToTheUiEvent('node.emphasize.byid', mutation.payload);
      else if (mutation.type === "deemphasize")
        skydiveInfraLayout.reactToTheUiEvent('node.deemphasize.byid', mutation.payload);
    });
  ...
  methods: {
    ...
    initInfraLayout: function() {
        ...
        var self = this;
        this.topologyManager = window.TopologyManager.getInstance();
        const skydiveInfraLayout = new window.TopologyORegistry.layouts.infra('.topology-d3-infra')
        if (!this.topologyManager.layoutRegistry.doesLayoutExist(skydiveInfraLayout)) {
            ...
            infraTopologyDataSource.e.on('broadcastMessage', (type, msg) => {
                ...
                self.topologyManager.layoutRegistry.setActive(skydiveInfraLayout);
            });
            ...
            this.topologyManager.addTopologyLayout(skydiveInfraLayout);
        }
    },
    ...
    removeInfraLayout: function() {
      ...
      this.topologyManager.layoutRegistry.removeLayout(this.infraLayout);
    },
